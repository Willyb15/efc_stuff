...
if (getParameterByName('searchid') != '' ) {
  ...
  var searchrdm = searchrdm2;
	...
	if (curtpage == 1) {
		...
		if (href.indexOf("?") >= 0) {
			history.replaceState({}, '', href + '&searchid=' + searchrdm + '&stst=' + getCookie("stestname"));
		} else {
			history.replaceState({}, '', href + '?searchid=' + searchrdm + '&stst=' + getCookie("stestname"));
		}
  	} else {
		var href = window.location.href;
		if (href.indexOf("?") >= 0) {
			history.replaceState({}, '', href + '&searchid=' + getCookie("searchid") + '&stst=' + getCookie("stestname"));
